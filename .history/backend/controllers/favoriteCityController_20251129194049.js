...
export const saveCity = async (req, res) => {
  const { userId, cityName, customName, isFavorite } = req.body;

  if (!userId) return res.status(400).json({ error: "userId is required" });
  if (!cityName) return res.status(400).json({ error: "City name is required" });

  try {
    const newCity = new FavoriteCity({ userId, cityName, customName, isFavorite: !!isFavorite });
    await newCity.save();
...
// ==================
// UPDATE CITY (including favorite)
// ==================
export const updateCity = async (req, res) => {
  const { cityId, cityName, customName, isFavorite } = req.body;
...
export const toggleFavorite = async (req, res) => {
  const { cityId, isFavorite } = req.body;
  if (!cityId) return res.status(400).json({ error: "cityId is required" });

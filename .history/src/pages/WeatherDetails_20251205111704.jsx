...
const WeatherDetails = () => {
  ...
  // 🔐 Control whether user is allowed to access the page
  const [allowed, setAllowed] = useState(false);

  // 🚨 STRICT LOGIN PROTECTION — blocks weather loading until verified
  useEffect(() => {
    try {
      ...
      if (!isLoggedIn) {
        ...
        setAllowed(true); // allow the user to view and fetch weather
      }
    } catch {
      navigate("/login");
    }
  ...
  // Prevent viewing page without city param
  useEffect(() => {
    if (!city) {
      navigate("/dashboard");
    }
  ...
  // ================================
  // Fetch 5-Day Forecast (runs ONLY when allowed)
  // ================================
  const fetchForecast = async (cityName) => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/weather/5days/${encodeURIComponent(
          cityName
        )}`
      );
  ...
  // Run fetch only AFTER login check passes
  useEffect(() => {
    if (allowed && city) {
      fetchForecast(city);
    }
            ...
            <div className="forecast-grid">
              {forecast.map((item, index) => {
                ...
                const temp = Math.round(
                  item.temp?.day || item.feels_like?.day
                );

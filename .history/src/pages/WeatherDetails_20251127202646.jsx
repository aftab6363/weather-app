...
const WeatherDetails = () => {
  ...
  const fetchForecast = async (cityName) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/weather/7days/${encodeURIComponent(cityName)}`);

      // If server sends HTML (404 page) you'll hit this non-ok branch
      if (!res.ok) {
        // try to parse JSON error if provided, otherwise throw generic
      ...
      <div className="weather-details-container">
        <div className="weather-details-card">
          <div className="details-header">
            <h1 className="city-title">7-Day Forecast — <span style={{color:"white"}}>{city}</span></h1>
